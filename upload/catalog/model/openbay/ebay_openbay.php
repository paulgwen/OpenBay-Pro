...
class ModelOpenbayEbayOpenbay extends Model{
	...
	private function updateOrderWithConfirmedData($order_id, $order) {
		...
		$this->session->data['shipping_address']['country_id'] = $user['country_id'];
		$this->session->data['shipping_address']['zone_id'] = $user['zone_id'];
		$this->session->data['payment_address']['country_id'] = $user['country_id'];
		$this->session->data['payment_address']['zone_id'] = $user['zone_id'];
