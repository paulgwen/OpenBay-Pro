...
final class Ebay
{
	...
	public function getProductId($ebay_item, $status = 0) {
		...
		$qry = $this->db->query("SELECT `product_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `ebay_item_id` = '".$ebay_item."'".$status_sql." ORDER BY `ebay_listing_id` DESC LIMIT 1");
    ...
    public function getEbayItemList($limit = 100, $page = 1, $filter = array()) {
        ...
        return $this->openbay_call('item/getItemListLimited/', array('page' => $page, 'limit' => $limit, 'filter' => $filter));
    }
}
?>