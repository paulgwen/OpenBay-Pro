...
final class Ebay {
	...
	public function getProductId($ebay_item, $status = 0) {
		...
		$qry = $this->db->query("SELECT `product_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `ebay_item_id` = '" . $this->db->escape($ebay_item) . "'" . $status_sql . " ORDER BY `ebay_listing_id` DESC LIMIT 1");
