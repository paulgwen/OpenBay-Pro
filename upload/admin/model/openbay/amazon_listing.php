...
class ModelOpenbayAmazonListing extends Model {
	...
	public function simpleListing($data) {
		$request = array(
			...
			'response_url' => $this->config->get('config_url') . 'index.php?route=openbay/amazon/listing',
	...
	public function doBulkSearch($search_data) {
		...
		$request_data = array(
			'search' => $search_data,
			'response_url' => $this->config->get('config_url') . 'index.php?route=openbay/amazon/search'
		);
