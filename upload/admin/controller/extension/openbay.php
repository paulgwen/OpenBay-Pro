...
class ControllerExtensionOpenbay extends Controller {
    ...
    public function orderList(){
		...
		$this->data['channels'] = array();
 		...
 		if ($this->config->get('play_status')) {
 			$this->data['channels'][] = array(
 				'module' => 'play',
 				'title' => $this->language->get('text_play'),
  			);
  		}
        ...
        if (isset($this->request->get['filter_channel'])) {
            $url .= '&channel=' . $this->request->get['filter_channel'];
        }
    ...
    public function orderListComplete(){
		...
		if ($this->config->get('amazon_status') == 1) {
			...
			foreach ($this->request->post['order_id'] as $order_id) {
				if ($this->request->post['channel'][$order_id] == 'Amazon EU') {
					...
					if ($this->config->get('openbay_amazon_order_status_shipped') == $this->request->post['order_status_id']) {
						...
						$this->model_amazon_amazon->updateAmazonOrderTracking($order_id, $carrier, $carrier_from_list, !empty($carrier) ? $this->request->post['tracking'][$order_id] : '');
					}
			...
			if ($orders) {
				$this->amazon->bulkUpdateOrders($orders);
			}
		}
		...
		if ($this->config->get('amazonus_status') == 1) {
			...
			foreach ($this->request->post['order_id'] as $order_id) {
				if ($this->request->post['channel'][$order_id] == 'Amazon US') {
					...
					if ($this->config->get('openbay_amazonus_order_status_shipped') == $this->request->post['order_status_id']) {
						...
						$this->model_amazonus_amazonus->updateAmazonusOrderTracking($order_id, $carrier, $carrier_from_list, !empty($carrier) ? $this->request->post['tracking'][$order_id] : '');
					}
			...
			if ($orders) {
				$this->amazonus->bulkUpdateOrders($orders);
			}
		}
